...
fn main() {
    ...
    if target_os == "windows" {
        ...
        println!(
            "cargo:warning=Building for unsupported OS: {target_os}. Some features may not work."
        );
    }
}
...
fn setup_unix_build() {
    ...
    config_content.push_str(&format!("pub const HAS_XORRISO: bool = {has_xorriso};\n"));
    config_content.push_str(&format!("pub const HAS_QEMU: bool = {has_qemu};\n"));
    config_content.push_str(&format!(
        "pub const HAS_VIRTUALBOX: bool = {has_virtualbox};\n"
    ));
