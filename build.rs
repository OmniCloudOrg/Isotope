...
fn main() {
    ...
    if target_os == "windows" {
        ...
        println!("cargo:warning=Building for unsupported OS: {}. Some features may not work.", target_os);
    }
}
...
fn setup_windows_build() {
    ...
    if !has_powershell {
        println!("cargo:warning=PowerShell not found. Some Windows-specific features may not work.");
    }
    ...
    if has_virtualbox {
        ...
        println!("cargo:warning=VirtualBox not found. VirtualBox-specific features will be disabled.");
    }
...
fn setup_unix_build() {
    ...
    config_content.push_str(&format!("pub const HAS_VIRTUALBOX: bool = {};\n", has_virtualbox));
