...
impl QemuProvider {
    ...
    fn get_monitor_socket(&self, instance: &VmInstance) -> PathBuf {
        self.working_dir
            .join(format!("{}-monitor.sock", instance.name))
    }
...
impl VmProviderTrait for QemuProvider {
    ...
    async fn start_vm(&self, instance: &mut VmInstance) -> Result<()> {
        ...
        cmd.args([
            "-m",
            &instance.config.memory_mb.to_string(),
            "-smp",
            &instance.config.cpus.to_string(),
            "-drive",
            &format!("file={},format=qcow2", disk_path.display()),
            "-monitor",
        ...
        let output = cmd.output().await.context("Failed to start QEMU VM")?;
    ...
    async fn stop_vm(&self, instance: &mut VmInstance) -> Result<()> {
        ...
        match self
            .send_monitor_command(instance, "system_powerdown")
            .await
        {
            Ok(_) => {
    ...
    async fn delete_vm(&self, instance: &mut VmInstance) -> Result<()> {
        ...
        if let Some(disk_path) = &instance.disk_path {
            if disk_path.exists() {
                std::fs::remove_file(disk_path).context("Failed to remove VM disk file")?;
            }
        }
    ...
    async fn attach_iso(&self, instance: &mut VmInstance, iso_path: &Path) -> Result<()> {
        ...
        if instance.is_running() {
            ...
            self.send_monitor_command(instance, &command)
                .await
    ...
    async fn detach_iso(&self, instance: &mut VmInstance) -> Result<()> {
        ...
        if instance.is_running() {
            self.send_monitor_command(instance, "change ide1-cd0 /dev/null")
                .await
    ...
    async fn create_snapshot(&self, instance: &VmInstance, snapshot_name: &str) -> Result<()> {
        ...
        if instance.is_running() {
            ...
            let output = Command::new("qemu-img")
                .args(["snapshot", "-c", snapshot_name, disk_path.to_str().unwrap()])
                .output()
    ...
    async fn restore_snapshot(&self, instance: &mut VmInstance, snapshot_name: &str) -> Result<()> {
        ...
        let output = Command::new("qemu-img")
            .args(["snapshot", "-a", snapshot_name, disk_path.to_str().unwrap()])
            .output()
    ...
    async fn capture_screen(&self, instance: &VmInstance) -> Result<DynamicImage> {
        ...
        let screenshot_path = self
            .working_dir
            .join(format!("{}-screenshot.ppm", instance.name));
...
impl QemuProvider {
    fn get_console_log_path(&self, instance: &VmInstance) -> PathBuf {
        self.working_dir
            .join(format!("{}-console.log", instance.name))
    }
