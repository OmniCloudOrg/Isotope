...
impl VmMetadata {
    ...
    pub fn load_from_current_dir() -> Result<Self> {
        ...
        let metadata: VmMetadata = serde_json::from_str(&content)
            .with_context(|| "Failed to parse .isometa file")?;
    ...
    pub fn cleanup_stale_entries(&mut self) {
        ...
        for key in to_remove {
            if let Some(entry) = self.vms.remove(&key) {
                info!("Cleaned up stale entry for VM {} (isotope file no longer exists)", entry.vm_name);
            }
        }
    }
